...
# Usage: 
# build_rgraphs.py -i <source file> -o <destination file (graph)> [-draw <destination file (graphml)>] [-ini] [-s]
# Requirements:
...
for i in range(len(SCC_list)): 
	...
	for j in range(len(attr)):
		v = attr[j]
		if GX.has_node(v): # check that v is still available
			# contract nodes from neighbours
			nb_list = list(GX.neighbors(v)) 
			pr_list = list(GX.predecessors(v))
			c_list = list(set(nb_list+pr_list)) # avoid duplicates
			for d in c_list:
				if d in attr: # contract within the same attractor only
					if SGraph[d].ini == 1: # mark as contraction from INI state
						SGraph[v].cini = 1	
					SGraph[d].contr = 1
					nx.contracted_nodes(GX,v,d,self_loops=False, copy=False) # in result v should remain as a representative vertice
...
while not_completed:
	...
	for e in L_edges:
		...
		if e in GX.edges and SGraph[s].attr == -1 and SGraph[s].dec == -1 and SGraph[d].attr == -1 and SGraph[d].dec == -1: #check that boring and that edge is still available
			...
			not_completed = True
		# the second case - edge between two identical swiching states, always contract	
		...
		if e in GX.edges and SGraph[s].attr == -1 and SGraph[s].dec == -1 and SGraph[d].attr != -1: # check that boring to attractors and that edge is still available
			...
			if (len(pr_list) == 0): # possibly a single vertex region, keep it as grey
				...
				if SGraph[s].ini == 1: # mark as contraction from INI state
					SGraph[d].cini = 1	
				SGraph[s].contr = 1
...
for (s,d) in Lall_edges:
	if (s,d) in GX.edges and SGraph[s].grey == 1: # edge is still available and source is grey edge
		...
		for p in pr_list:
			if p!=s and p in GX.nodes and SGraph[s].grey == 1 and GX.neighbors(s) == GX.neighbors(p): # a distinct predecessor that is also grey and with the same reachability modes
				if SGraph[p].ini == 1: # mark as contraction from INI state
					SGraph[s].cini = 1	
				SGraph[p].contr = 1	
...
if args['fname_outdraw']:
	...
	for v in GX.nodes:
		...
		if SGraph[vid].dec != -1: # add switching condition
		...
		if SGraph[vid].grey != -1: # add grey
			grey_colour = '#C0C0C0' #default
			if SGraph[vid].cini != - 1:  # state includes INI position
				grey_colour = '#FADA5E'
			diagram.add_node(str(vid),label=str(vid),attributes={'Shape': {'type': 'rectangle'}, 'NodeLabel' : {'alignment' : 'center', 'autoSizePolicy' : 'content', 'fontFamily' : 'Dialog', 'fontSize' : '20', 'fontStyle' : 'bold'}, 'Fill' : {'color': grey_colour}, 'BorderStyle' : {'width' : '3.0'}}) 
	# RG edges
	for (s,d) in GX.edges:
		sid = IDs[s]
		did = IDs[d]
